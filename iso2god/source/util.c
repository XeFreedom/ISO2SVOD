...
u16 getLe16(VOID* inval)
{
	unsigned char* data = (unsigned char*)inval;
	u16 ret = (data[1] & 0xFF) << 8;
	ret |= (data[0] & 0xFF);
	return ret;
}

u32 getLe32(VOID* inval)
{
	unsigned char* data = (unsigned char*)inval;
...
void setBe16(VOID* inval, u16 val)
{
	unsigned char* data = (unsigned char*)inval;
	data[0] = ((val >> 8) & 0xFF);
	data[1] = (val&0xFF);
...
void setBe32(VOID* inval, u32 val)
{
	unsigned char* data = (unsigned char*)inval;
	data[0] = ((val >> 24) & 0xFF);
	data[1] = ((val>>16)&0xFF);
...
void setBe64(VOID* inval, u64 val)
{
	unsigned char* data = (unsigned char*)inval;
...
int isDirExist(const char* dirname)
{
	struct stat sti;
	char dtmp[MAXPATHLEN];
	// windows seems to need the trailing \\ removed for this to work
	int len = strlen(dirname);
	strcpy_s(dtmp, MAXPATHLEN, dirname);
	if (dtmp[len - 1] == '\\')
	{
		dtmp[len - 1] = 0;
		len--;
...
// c:\\some\\long\\ass\\path
BOOL doRecursiveCreateDir(char* dirname, int len)
{
	int cur = 0;
...
BOOL createDirRecursive(const char* dirname)
{
	char dtmp[MAXPATHLEN];
	int len = strlen(dirname);
	strcpy_s(dtmp, MAXPATHLEN, dirname);
	...
	return doRecursiveCreateDir(dtmp, len);
}
