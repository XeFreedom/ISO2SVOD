...
static void deskey(const unsigned char *key, short edf, u32 *keyout)
{
	u32 i, j, l, m, n, kn[32];
...
int des3_setup(const unsigned char *key, int keylen, int num_rounds, XECRYPT_DES3_STATE *skey)
{
	...
	deskey(key+8,  DE1, skey->ek[1]);
	deskey(key+16, EN0, skey->ek[2]);

	deskey(key,    DE1, skey->dk[2]);
	deskey(key+8,  EN0, skey->dk[1]);
	deskey(key+16, DE1, skey->dk[0]);
...
/**
  ...
  @return CRYPT_OK if successful
*/
int des_ecb_encrypt(const unsigned char *pt, unsigned char *ct, XECRYPT_DES_STATE *skey)
{
	u32 work[2];
	LTC_ARGCHK(pt   != NULL);
	LTC_ARGCHK(ct   != NULL);
...
/**
  ...
  @return CRYPT_OK if successful
*/
int des_ecb_decrypt(const unsigned char *ct, unsigned char *pt, XECRYPT_DES_STATE *skey)
{
	u32 work[2];
	LTC_ARGCHK(pt   != NULL);
	LTC_ARGCHK(ct   != NULL);
...
/**
  ...
  @return CRYPT_OK if successful
*/
int des3_ecb_encrypt(const unsigned char *pt, unsigned char *ct, XECRYPT_DES3_STATE *skey)
{
	u32 work[2];
	...
	LOAD32H(work[0], pt+0);
	LOAD32H(work[1], pt+4);
	desfunc(work, skey->ek[0]);
	desfunc(work, skey->ek[1]);
	desfunc(work, skey->ek[2]);
	STORE32H(work[0],ct+0);
	STORE32H(work[1],ct+4);
	return CRYPT_OK;
}
...
/**
  ...
  @return CRYPT_OK if successful
*/
int des3_ecb_decrypt(const unsigned char *ct, unsigned char *pt, XECRYPT_DES3_STATE *skey)
{
	u32 work[2];
	LTC_ARGCHK(pt   != NULL);
	LTC_ARGCHK(ct   != NULL);
	...
	LOAD32H(work[0], ct+0);
	LOAD32H(work[1], ct+4);
	desfunc(work, skey->dk[0]);
	desfunc(work, skey->dk[1]);
	desfunc(work, skey->dk[2]);
	STORE32H(work[0],pt+0);
	STORE32H(work[1],pt+4);
	return CRYPT_OK;
}
